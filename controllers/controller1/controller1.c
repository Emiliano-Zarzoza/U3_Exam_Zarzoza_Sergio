...
void ManualMode(){
    ...
    PosSensor2 = wb_position_sensor_get_value(encoder[1]);
    PosSensor3 = wb_position_sensor_get_value(encoder[2]);
    printf("encoder_1: %lf\n", encoder[0]);
    printf("encoder_2: %lf\n", encoder[1]);
    printf("encoder_3: %lf\n", encoder[2]);
  ...
  void AutonomousMode(){
    ...
    if(distSensor1_value < distSensor2_value && dist_left_sensor1 < 200) {
      wb_motor_set_velocity(wheels[0], 6.66);
      wb_motor_set_velocity(wheels[1], 0);
